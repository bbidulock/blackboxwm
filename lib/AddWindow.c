...
static Bool addWindow(Display *d, XEvent *e, XPointer arg) {
  if (e->type == ClientMessage)
    if (e->xclient.message_type == _BLACKBOX_MESSAGE &&
        e->xclient.format == 32)
      if (e->xclient.data.l[0] == __blackbox_addTopLevelWindow &&
...
Bool BlackboxAddWindow(Display *display, Window window) {
  if (display && window) {
    Atom atom_return;
    ...
    if (data) {
      ...
      if (format_return == 32 && atom_return == _BLACKBOX_CONTROL) {
        ...
        e.xclient.format = 32;
        e.xclient.data.l[0] = __blackbox_addTopLevelWindow;
        e.xclient.data.l[1] = window;
        e.xclient.data.l[2] = 0l;
        e.xclient.data.l[3] = 0l;
        e.xclient.data.l[4] = 0l;
	
        // tell blackbox about our toplevel window... and wait for confirmation
        XSendEvent(display, controlWindow, False, NoEventMask, &e);
        XIfEvent(display, &e, addWindow, (XPointer) &window);
        return True;
      }
    }
  } else
    puts("AddWindow.c: BlackboxAddWindow(): invalid display/window arguments");
